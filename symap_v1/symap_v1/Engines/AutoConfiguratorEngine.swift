...
enum AutoConfiguratorEngine {
    ...
    static let specs: [String: ModelSpec] = [
        ...
        "timbau": ModelSpec(baseBridge: 16.2, baseWidth: 140.0, limits: ModelLimits(bridgePlus: 4.0, bridgeMinus: 4.0, nasal: 2.0, ferradura: 0.0, larguraR: 4.0, larguraA: 4.0, verticalR: 2.0, verticalA: 2.0))
    ]
    ...
    /// Calcula os pesos (0.0 a 1.0) para as Shape Keys (Morphers) baseados na biometria do paciente
        static func calculateMorphWeights(keyword: String, faceWidth: Float, bridgeWidth: Float, nasalProfile: String, faceShape: String) -> [String: Float] {
            ...
            // 🔴 4. NOVO: Estética Vertical baseada no Formato do Rosto (Visagismo)
            if faceShape.contains("Longo") {
                weights["Vertical_a"] = 0.8 // Estica a lente para baixo para equilibrar rostos longos
            } else if faceShape.contains("Redondo") {
                weights["Vertical_r"] = 0.8 // Achata a lente para afinar rostos redondos
            }
            
            // 🔴 5. NOVO: Ergonomia de Ferradura para narizes estreitos
            if bridgeWidth < 15.0 {
                weights["Ferradura"] = 0.9 // Engrossa a ponte para dar volume ao nariz
            }
